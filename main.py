...
def player_movec(icon):
    ...
    def dup():
        ...
        if(board[0]==icon and board[1]==icon) or (board[5]==icon and board[8]==icon) or (board[4]==icon and board[6]==icon):
            ...
            n1=6
            ai()
        elif(board[3]==icon and board[5]==icon) or (board[1]==icon and board[7]==icon) or (board[0]==icon and board[8]==icon) or\
             (board[2]==icon and board[6]==icon):
            n1=5
            ai()
    ...
    if ch==2:
        while True:
           ...
           while True:
                ...
                if is_victory("O"):
                    print_board()
                    print("Computer(O) wins.. :) Congratulations")
                    sys.exit()
                elif is_draw():
                    print("It's a draw!")
                    sys.exit()
